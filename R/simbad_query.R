...
simbad_query <- function(objects = NULL, params = NULL, col_names = NULL) {
  ...
  get_data <- GET(base_sim_script_url,
                        query = list(script = paste0('format object \"', params, '\"\n',
                                                     "output console=off script=off\n", objects)))

  simbad_data <- content(get_data)
  ...
  simbad_data <- data.frame(simbad_data, stringsAsFactors = FALSE)
  simbad_data <- separate(simbad_data, simbad_data, col_names, sep = ';', convert = TRUE)
